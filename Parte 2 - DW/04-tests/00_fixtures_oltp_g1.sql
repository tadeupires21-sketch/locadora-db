...
-- ---- Reset do log de extração ----
-- O log de extração (stg.log_extracao) alimenta a janela incremental do
-- ETL: MAX(dt_extracao) define o baseline a partir do qual novos registros
-- são extraídos. Em execuções repetidas da suíte de testes, o log da rodada
-- anterior (com dt_extracao = agora) deslocaria a janela para o presente e
-- as locações sintéticas (created_at = CURRENT_DATE - 5) ficariam de fora,
-- zerando stg.locacao. Limpar o log aqui restaura o fallback de 7 dias e
-- torna o teste determinístico, independente de quantas vezes rodou antes.
-- TRUNCATE protegido para o caso de a tabela ainda não existir.
DO $$
BEGIN
    IF to_regclass('stg.log_extracao') IS NOT NULL THEN
        TRUNCATE TABLE stg.log_extracao;
    END IF;
END $$;
